...
var Backoff = function(type) {
    ...
    this.calculateDelay = Backoff._strategies[type].calculateDelay;
...
Backoff._strategies = {
    ...
    'network': {
        ...
        calculateDelay: function(attempts) {
            var delay = Backoff._strategies['network'].MS*attempts;
            if (delay > Backoff._strategies['network'].MAX) {
                return Backoff._strategies['network'].MAX;
            } else {
                return delay;
            }
        }
    ...
    'http': {
        ...
        calculateDelay: function(attempts) {
            var delay = Backoff._strategies['http'].MS*Math.pow(2, attempts-1);
            if (delay > Backoff._strategies['http'].MAX) {
                return Backoff._strategies['http'].MAX;
            } else {
                return delay;
            }
        }
    ...
    'http420': {
        ...
        calculateDelay: function(attempts) {
            var delay = Backoff._strategies['http420'].MS*Math.pow(2, attempts-1);
            return delay;
        }
    }
